...
# instantiate persistent client using new Chroma API
...
def add_documents(ids: List[str], documents: List[str], embeddings, metadatas: List[Dict[str, Any]]):
    coll = _get_or_create_collection()
    # embeddings must be python lists
...
def query_embeddings(embedding, n_results: int = 4):
    coll = _get_or_create_collection()
    res = coll.query(query_embeddings=[embedding.tolist() if hasattr(embedding, "tolist") else embedding],
                     n_results=n_results,
                     include=["documents", "metadatas", "distances"])
    # return flattened results for first query
