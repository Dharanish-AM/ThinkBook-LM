...
def chunk_text_tokenwise(text: str, chunk_size: int, overlap: int) -> List[str]:
    # uses tiktoken for more accurate chunking by tokens
...
def chunk_text_charwise(text: str, chunk_size_chars: int = 3000, overlap_chars: int = 500) -> List[str]:
    chunks = []
    n = len(text)
    start = 0
    while start < n:
...
def chunk_text(text: str):
    ...
    # Fallback heuristic: approximate chars ~ tokens * 4
